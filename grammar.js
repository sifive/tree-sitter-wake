// Turns off all Unicode not used by the standard library
const disable_unicode = true;
...
function binary_op(left, op, repeat, term) {
  if (left) {
    return seq(
      field('lhs',      repeat),
      field('operator', op),
...
function make_ops($, kind) {
  const unary  = 'unary_'  + kind;
  const binary = 'binary_' + kind;
  $[unary]  = $ => 'unreachable';
  $[binary] = $ => 'unreachable';
...
function operator(category) {
  const terms = [op_mod, disable_unicode?"":"*", category, op_body, "*"];
...
let rules = {
  ...
  _full_expression: $ => choice(
    $.lambda_expression,
    $.if_expression,
    $._operator_expression), // created by make_ops
  ...
  dot_op:         $ => operator(                 '[.]'),
  composition_op: $ => operator(disable_unicode? '[∘]'            : '[∘⊚⋆⦾⧇]'),
  unary_fn_op:    $ => operator(                 '[√∛∜]'),
  exponent_op:    $ => operator(                 '\\^'),
  muldiv_op:      $ => operator(disable_unicode? '[*/%∏]'         : '[*/%∏⋂⨀⨂⨅⨉∐÷⊘⟌⦸⦼⧶⧷⨸⫻⫽×∙∩≀⊓⊗⊙⊛⊠⊡⋄⋅⋇⋈⋉⋊⋋⋌⋒⟐⟕⟖⟗⟡⦁⦻⦿⧆⧑⧒⧓⧔⧕⧢⨝⨯⨰⨱⨲⨳⨴⨵⨶⨷⨻⨼⨽⩀⩃⩄⩋⩍⩎]'),
  addsub_op:      $ => operator(disable_unicode? '[-+~∑∪⊎∩∖]'     : '[-+~∑∫∮∱∲∳⋃⨁⨃⨄⨆⨊⨋⨍⨎⨏⨐⨑⨒⨓⨔⨕⨖⨗⨘⨙⨚⨛⨜⫿¬±∓∔∪∸∸∹∺∻≂⊌⊍⊎⊔⊕⊖⊞⊟⊹⊻⋓⧺⧻⧾⧿⨢⨣⨤⨥⨦⨧⨨⨩⨪⨫⨬⨭⨮⨹⨺⨿⩁⩂⩅⩊⩌⩏⩐⩪⩫⫬⫭⫾]'),
  comparison_op:  $ => operator(disable_unicode? '[<>∈∉∋∌⊆⊇⊉⊈⊊⊋]' : '[<>≤≦≨≪≮≰≲≴≶≸≺≼≾⊀⊂⊄⊆⊈⊊⊏⊑⊰⊲⊴⊷⋐⋖⋘⋚⋜⋞⋠⋢⋤⋦⋨⋪⋬⟃⟈⧀⧏⧡⩹⩻⩽⩿⪁⪃⪅⪇⪉⪋⪍⪏⪑⪓⪕⪗⪙⪛⪝⪟⪡⪣⪦⪨⪪⪬⪯⪱⪳⪵⪷⪹⪻⪽⪿⫁⫃⫅⫇⫉⫋⫍⫏⫑⫓⫕⫷⫹≥≧≩≫≯≱≳≵≷≹≻≽≿⊁⊃⊅⊇⊉⊋⊐⊒⊱⊳⊵⊶⋑⋗⋙⋛⋝⋟⋡⋣⋥⋧⋩⋫⋭⟄⟉⧁⧐⩺⩼⩾⪀⪂⪄⪆⪈⪊⪌⪎⪐⪒⪔⪖⪘⪚⪜⪞⪠⪢⪧⪩⪫⪭⪰⪲⪴⪶⪸⪺⪼⪾⫀⫂⫄⫆⫈⫊⫌⫎⫐⫒⫔⫖⫸⫺∈∉∋∌∝∟∠∡∢∥∦≬⊾⊿⋔⋲⋳⋵⋶⋸⋹⋺⋻⋽⋿⍼⟊⟒⦛⦜⦝⦞⦟⦠⦡⦢⦣⦤⦥⦦⦧⦨⦩⦪⦫⦬⦭⦮⦯⦶⦷⦹⦺⩤⩥⫙⫚⫛⫝̸⫝⫡⫮⫲⫳⫴⫵⫶⫼]'),
  inequality_op:  $ => operator(disable_unicode? '[!=]'           : '[!==≃≄≅≆≇≈≉≊≋≌≍≎≏≐≑≒≓≔≕≖≗≘≙≚≛≜≝≞≟≠≡≢≣≭⊜⋍⋕⧂⧃⧎⧣⧤⧥⧦⧧⩆⩇⩈⩉⩙⩦⩧⩨⩩⩬⩭⩮⩯⩰⩱⩲⩳⩷⩸⪤⪥⪮⫗⫘]'),
  and_op:         $ => operator(disable_unicode? '[&]'            : '[&∧⊼⋏⟎⟑⨇⩑⩓⩕⩘⩚⩜⩞⩞⩟⩟⩠⩠⋀]'),
  or_op:          $ => operator(disable_unicode? '[|]'            : '[|∨⊽⋎⟇⟏⨈⩒⩔⩖⩗⩛⩝⩡⩢⩣⋁]'),
  currency_op:    $ => operator(disable_unicode? '[$]'            : Sc),
  lr_arrow_op:    $ => operator(disable_unicode? '[←→]'           : '[←↑↚⇷⇺⇽⊣⊥⟣⟥⟰⟲⟵⟸⟻⟽⤂⤆⤉⤊⤌⤎⤒⤙⤛⤝⤟⤣⤦⤧⤪⤱⤲⤴⤶⤺⤽⤾⥀⥃⥄⥆⥉⥒⥔⥖⥘⥚⥜⥞⥠⥢⥣⥪⥫⥳⥶⥷⥺⥻⥼⥾⫣⫤⫥⫨⫫⬰⬱⬲⬳⬴⬵⬶⬷⬸⬹⬺⬻⬼⬽⬾⬿⭀⭁⭂⭉⭊⭋→↓↛↠↣↦⇏⇒⇴⇶⇸⇻⇾⊢⊤⊦⊧⊨⊩⊪⊫⊬⊭⊮⊯⊺⟢⟤⟱⟳⟴⟶⟹⟼⟾⟿⤀⤁⤃⤅⤇⤈⤋⤍⤏⤐⤑⤓⤔⤕⤖⤗⤘⤚⤜⤞⤠⤤⤥⤨⤩⤭⤮⤯⤰⤳⤵⤷⤸⤹⤻⤼⤿⥁⥂⥅⥇⥓⥕⥗⥙⥛⥝⥟⥡⥤⥥⥬⥭⥰⥱⥲⥴⥵⥸⥹⥽⥿⧴⫢⫦⫧⫪⭃⭄⭇⭈⭌]'),
  bi_arrow_op:    $ => operator(disable_unicode? '[↔]'            : '[↔↮⇎⇔⇵⇹⇼⇿⟚⟛⟠⟷⟺⤄⤡⤢⤫⤬⥈⥊⥋⥌⥍⥎⥏⥐⥑⥦⥧⥨⥩⥮⥯⫩]'),
  quantifier_op:  $ => operator(disable_unicode? '[∀]'            : '[∀∁∃∄∎∴∵∷]'),
  colon_op:       $ => operator(                 '[:]'),
  comma_op:       $ => operator(                 '[,]'),

  _low_identifier:  $ => {
    const terms = [id_mod, disable_unicode?"":"*", id_low, id_body, "*"];
    return new RegExp(terms.join(''), 'u');
  },

  _high_identifier: $ => {
    const terms = [id_mod, disable_unicode?"":"*", id_high, id_body, "*"];
    return new RegExp(terms.join(''), 'u');
  }
};

make_ops(rules, 'expression');
make_ops(rules, 'pattern');
make_ops(rules, 'type');
